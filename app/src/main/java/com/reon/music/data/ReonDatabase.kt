...
/**
 * SourceKind enforces split quality story: LOCAL_HIRES is real FLAC/ALAC/WAV,
 * YT_STREAM is Opus/AAC 128-160kbps and must never be labeled Hi-Res.
 */
object SourceKind {
    const val LOCAL_HIRES = "LOCAL_HIRES"
    const val YT_STREAM = "YT_STREAM"
}
...
data class TrackEntity(
    ...
    val badge: String = "",
    val artSeed: Int = 1,
    // Phase 2: backend stream fields (v2)
    val videoId: String = "",
...
interface ReonDao {
    ...
    suspend fun getTrackByVideoId(videoId: String): TrackEntity?

    @Query("UPDATE tracks SET streamUrl = :url, streamExpiresAt = :expiresAt WHERE id = :id")
    suspend fun updateStream(id: String, url: String, expiresAt: Long)
